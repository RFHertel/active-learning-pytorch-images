...
Changes to address workers/threads issues:
- Forced debug config when running in VS Code debug (improved detection).
- persistent_workers=True in prod, False in debug (as per user request).
- Set persistent_workers based on config for ALL loaders.
- Added more robust cleanup in finally blocks.
- Set num_workers=0 in debug to avoid multiprocessing conflicts with debuggers.
- Added prints for debugging the selection process.
"""
...
DEBUG_CONFIG = {
    ...
    'num_workers': 0,          # No multiprocessing in debug to avoid debugger conflicts
    'persistent_workers': False,  # Disabled in debug
...
PROD_CONFIG = {
    ...
    'num_runs': 1,
    'num_workers': 0,
    'persistent_workers': False,  # Enabled in prod for performance
    ...
    def compute_all_uncertainties(self, unlabeled_loader):
        ...
        with torch.no_grad():
            for batch_idx, (data, _) in enumerate(unlabeled_loader):
                print(f"Processing batch {batch_idx + 1}/{len(unlabeled_loader)} in compute_all_uncertainties")  # Debug print
...
class OptimizedBALDStrategy(QueryStrategy):
    ...
    def select(self, unlabeled_loader, n_samples):
        with monitor.timer("BALD Selection"):
            ...
            with torch.no_grad():
                for batch_idx, (data, _) in enumerate(unlabeled_loader):
                    print(f"Processing batch {batch_idx + 1}/{len(unlabeled_loader)} in BALD select")  # Debug print
                    data = data.to(self.device, non_blocking=True)
                    batch_size = data.size(0)
...
class OptimizedActiveLearningPipeline:
    ...
    def __init__(self, config=None):
        ...
        self.config = {
            ...
            'persistent_workers': False,  # Default True, but overridden by debug/prod config
    ...
    def _setup_data(self):
        ...
        # Create persistent test loader (reused)
                ...
                if iteration < self.config['max_iterations'] - 1 and len(al_dataset.unlabeled_indices) > 0:
                    ...
                    with self.monitor.timer(f"Query Selection (Iter {iteration+1})"):
                        if strategy_name == 'random':
                            ...
                            print(f"Starting selection for {strategy_name}")  # Debug print
                            selected_relative, _ = strategy.select(unlabeled_loader, query_size)
                            print(f"Selection completed for {strategy_name}")  # Debug print
    ...
    def _create_plots(self, stats):
        ...
        # Save and open
        fig.write_html('optimized_active_learning_comparison.html')
        webbrowser.open('optimized_active_learning_comparison.html')
...
def main():
   ...
   try:
       ...
       print("\n" + "="*80)
       print("✅ Experiment complete! Check 'optimized_active_learning_comparison.html' for visualizations.")
       print("="*80)
